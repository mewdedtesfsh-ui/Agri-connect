...
const ManageMarkets = () => {
  ...
  const handleDelete = async (id) => {
    setConfirmDialog({
      isOpen: true,
      title: t('extension.deleteMarket'),
      message: t('extension.confirmDeleteMarket'),
      type: 'danger',
      onConfirm: async () => {
        try {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">{t('extension.manageMarkets')}</h1>
          <button
            onClick={() => setShowForm(!showForm)}
            className="bg-primary text-white px-4 py-2 rounded hover:bg-secondary"
          >
            {showForm ? t('common.cancel') : t('extension.addMarket')}
          </button>
        </div>

        {showForm && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-bold mb-4">{editingMarket ? t('extension.editMarket') : t('extension.addNewMarket')}</h2>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-gray-700 mb-2">{t('extension.marketName')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-gray-700 mb-2">{t('extension.region')}</label>
                  <input
                    type="text"
                    className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"

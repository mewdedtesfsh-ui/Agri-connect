...
const ManagePrices = () => {
  ...
  const handleDelete = async (id) => {
    setConfirmDialog({
      isOpen: true,
      title: t('admin.deletePrice'),
      message: t('admin.confirmDeletePrice'),
      type: 'danger',
      onConfirm: async () => {
        try {
    ...
    <AdminLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">{t('admin.managePrices')}</h1>
          <button
            onClick={() => setShowForm(!showForm)}
            className="bg-primary text-white px-4 py-2 rounded hover:bg-secondary"
          >
            {showForm ? t('common.cancel') : t('admin.addPrice')}
          </button>
        </div>

        {showForm && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-bold mb-4">{t('admin.addNewPrice')}</h2>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-gray-700 mb-2">{t('admin.crop')}</label>
                  <select
                ...
                <div>
                  <label className="block text-gray-700 mb-2">{t('admin.market')}</label>
                  <select
                    className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                ...
                <div>
                  <label className="block text-gray-700 mb-2">{t('admin.priceEtb')}</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0.01"
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-100">
                <tr>
                  <th className="text-left py-3 px-4">{t('admin.crop')}</th>
                  <th className="text-left py-3 px-4">{t('admin.market')}</th>
                  <th className="text-left py-3 px-4">{t('admin.region')}</th>
                  <th className="text-left py-3 px-4">{t('admin.priceEtb')}</th>
                  <th className="text-left py-3 px-4">{t('admin.lastUpdated')}</th>
